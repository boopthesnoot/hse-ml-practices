...
def divide_df(all_data, num_of_test):
    # Returns divided dfs of training and test set
    return all_data.loc[:num_of_test], all_data.loc[num_of_test:].drop(["Survived"], axis=1)


def preprocess(path_to_train, path_to_test, path_to_out):
    df_train = pd.read_csv(path_to_train)
    df_test = pd.read_csv(path_to_test)
    df_all = concat_df(df_train, df_test)
    split_value = df_train.shape[0]
    ...
    df_all['Survived'] = df_all['Survived'].fillna(0)
    ...
    def display_pclass_dist(percentages):
        ...
        plt.savefig("reports/figures/pclass_dist.png")
        # plt.show()
    ...
    def display_surv_dist(percentages):
        ...
        plt.savefig("reports/figures/surv_dist.png")
        # plt.show()
    ...
    df_train, df_test = divide_df(df_all, split_value)
    dfs = [df_train, df_test]
    ...
    df_train = df_all.loc[:split_value]
    df_test = df_all.loc[split_value:]
    dfs = [df_train, df_test]
    ...
    pd.DataFrame(X_train).to_csv(os.path.join(path_to_out, "X_train.csv"), index=False, header=False)
    pd.DataFrame(X_test).to_csv(os.path.join(path_to_out, "X_test.csv"), index=False, header=False)
    pd.DataFrame(y_train).to_csv(os.path.join(path_to_out, "y_train.csv"), index=False)
    df_test.to_csv(os.path.join(path_to_out, "df_test.csv"), index=False)
    df_all.to_csv(os.path.join(path_to_out, "df_all.csv"), index=False)
