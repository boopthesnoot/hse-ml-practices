...
def get_prediction(path_to_dir, path_to_out):
    ...
    X_train = np.loadtxt(os.path.join(path_to_dir, "X_train.csv"), delimiter=",")
    X_test = np.loadtxt(os.path.join(path_to_dir, "X_test.csv"), delimiter=",")
    y_train = np.loadtxt(os.path.join(path_to_dir, "y_train.csv"))
    df_test = pd.read_csv(os.path.join(path_to_dir, "df_test.csv"))
    df_all = pd.read_csv(os.path.join(path_to_dir, "df_all.csv"))
    y_train = y_train[1::]
    N = 5
    oob = 0
    probs = pd.DataFrame(
    ...
    submission_df.to_csv(os.path.join(path_to_out), header=True, index=False)
